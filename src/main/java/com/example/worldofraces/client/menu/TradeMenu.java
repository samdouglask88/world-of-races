...
import com.example.worldofraces.trade.TradePriceRegistry;
import com.example.worldofraces.trade.TradeService;
...
public final class TradeMenu extends AbstractContainerMenu {
    ...
    private TradeMenu(int id, Inventory inventory, RaceEntity npc) {
        ...
        addDataSlot(sync(() -> {
            if(inventory.player.level() instanceof net.minecraft.server.level.ServerLevel level&&npc.getPersonId()!=null)
                return com.example.worldofraces.society.HumanSocietySavedData.get(level).getPerson(npc.getPersonId())
    ...
    public RaceEntity getNpc(){return npc;} public int getMode(){return mode;} public int getAmount(){return amount;}
    ...
    private static RaceEntity findNpc(Inventory inv,int id){if(inv.player.level().getEntity(id)instanceof RaceEntity npc)return npc;throw new IllegalStateException("NPC do comercio nao encontrado");}
